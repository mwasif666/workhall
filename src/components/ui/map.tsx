...
function Map({ children, styles, ...props }: MapProps) {
  ...
  useEffect(() => {
    ...
    const mapStyle =
      (resolvedTheme ?? "light") === "dark" ? mapStyles.dark : mapStyles.light;
...
function MapMarker({
  ...
  }, [draggable, isLoaded, latitude, longitude, map, onClick, onDrag, onDragEnd, onDragStart, onMouseEnter, onMouseLeave]);
...
function ControlGroup({ children }: { children: ReactNode }) {
  return (
    <div className="overflow-hidden rounded-[18px] border border-black/10 bg-white/92 shadow-[0_18px_40px_rgba(15,23,42,0.16)] backdrop-blur-md [&>button:not(:last-child)]:border-b [&>button:not(:last-child)]:border-black/8">
      {children}
    </div>
  );
}
...
function ControlButton({
  ...
  return (
    <button
      ...
      type="button"
      className={cn(
        "flex size-11 items-center justify-center text-[#111111] transition-colors hover:bg-black/5 active:bg-black/10",
...
function MapPopup({
  ...
  }, [container, latitude, longitude, map]);
